...
  return text.length > 0 ? cyrMatches.length / text.length : 0;
}

// Normalize Cyrillic language detection for short texts
function normalizeCyrillicDetection(text, detectedLang) {
  // Only process if text has significant Cyrillic content
  const ratio = cyrillicRatio(text);
  if (ratio < 0.6) {
    return detectedLang; // Not primarily Cyrillic
  }
  
  // Short texts are unreliable for detection
  const shortText = text.trim().length < 12;
  
  // All Cyrillic languages except Russian (ru)
  // Override short texts to Russian as it's the most common Cyrillic language
  const cyrillicLangs = [
    'uk',  // Ukrainian
    'be',  // Belarusian
    'bg',  // Bulgarian
...
  }
  
  // Trust Google for longer texts or other languages (including Ukrainian, Belarusian, etc.)
  return detectedLang;
}
...
    line2Parts.push(responseTimeSec);
    
